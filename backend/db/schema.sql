...
CREATE TABLE public.property_cost_profiles (id uuid DEFAULT gen_random_uuid() NOT NULL, property_id uuid NOT NULL, fixed_stay_cost numeric DEFAULT 0 NOT NULL, cost_per_night numeric DEFAULT 0 NOT NULL, created_at timestamp with time zone DEFAULT timezone('utc'::text, now()), updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()));
...
CREATE TABLE public.properties (
  ...
  display_id text UNIQUE,
  category text NOT NULL DEFAULT 'Villa'::text,
...
CREATE TABLE public.property_date_blocks (
  ...
  created_at timestamp with time zone DEFAULT timezone('utc'::text, now()),
  created_by uuid,
  CONSTRAINT property_date_blocks_pkey PRIMARY KEY (id),
  CONSTRAINT property_date_blocks_property_id_fkey FOREIGN KEY (property_id) REFERENCES public.properties(id),
  CONSTRAINT property_date_blocks_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id)
);
CREATE TABLE public.property_cost_profiles (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  property_id uuid NOT NULL,
  ...
  CONSTRAINT property_cost_profiles_pkey PRIMARY KEY (id),
  CONSTRAINT property_cost_profiles_property_id_key UNIQUE (property_id),
  CONSTRAINT property_cost_profiles_property_id_fkey FOREIGN KEY (property_id) REFERENCES public.properties(id) ON DELETE CASCADE
);
CREATE TABLE public.reservation_profitability (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  booking_id uuid NOT NULL,
  ...
  CONSTRAINT reservation_profitability_booking_property_key UNIQUE (booking_id, property_id),
  ...
  CONSTRAINT reservation_profitability_property_id_fkey FOREIGN KEY (property_id) REFERENCES public.properties(id) ON DELETE CASCADE
);